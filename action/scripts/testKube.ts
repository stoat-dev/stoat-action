...
function convertToFileTree(loggables: KubeLoggable[]): FileTree {
  ...
  const namespaceNodes = Array.from(namespaces).map(
    (ns) =>
      ({
        name: ns,
        type: 'directory',
  ...
  for (const loggable of loggables) {
    ...
    (namespaceNode.children.find((n) => n.name === loggable.pod)! as Directory).children.push({
...
(async () => {
  ...
  for (const namespace of namespaces) {
    await kube.listNamespacedPod(namespace).then((res: { response: http.IncomingMessage; body: V1PodList }) => {
