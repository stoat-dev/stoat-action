...
The `static_hosting` key contains the required `path`. This path is specified from the root of the Git repository and can
...
That means, in your [comment template](./templating) you can use any metadata value or value under `static_hosting` to render the comment.

For example, the following template could be used to add a link to the file in the Stoat comment:
```handlebars
[{{ tasks.docs.metadata.name }}]({{ tasks.docs.static_hosting.link }})
```
...
```handlebars
![{{ tasks.taskid.metadata.name }}]({{ tasks.taskid.static_hosting.link }})
```
